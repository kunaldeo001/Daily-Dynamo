...
export function TaskItem({ task, onToggle, onDelete }: TaskItemProps) {
  ...
  return (
    <div className="space-y-2 perspective-1000">
      <li
        className={cn(
          'group flex items-center gap-3 p-3 rounded-xl bg-card border border-border/50 transition-all duration-500 animate-in fade-in slide-in-from-top-2 hover:shadow-xl relative overflow-hidden preserve-3d hover:translate-z-4 hover:rotate-x-1 hover:rotate-y-1',
          task.isWhimsical && "bg-gradient-to-r from-card to-accent/5 border-accent/30",
          task.isMainQuest && !task.isCompleted && "border-yellow-500/40 bg-yellow-500/5 shadow-yellow-500/10",
          {
            'opacity-60 grayscale-[0.5] translate-y-1 scale-[0.98] rotate-x-2': task.isCompleted,
      ...
      {showBreakdown && task.breakdown && (
        <div className="ml-8 p-3 bg-primary/5 rounded-lg border border-primary/10 text-xs text-primary-foreground/80 space-y-3 animate-in slide-in-from-top-2 duration-300 shadow-inner preserve-3d">
          ...
          <div className="space-y-2">
            {task.breakdown.split('\n').map((step, i) => (
              <div key={i} className="flex items-start gap-2 group/step hover:translate-x-1 transition-transform">
